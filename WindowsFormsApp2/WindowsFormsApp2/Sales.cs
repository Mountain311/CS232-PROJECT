...
namespace WindowsFormsApp2
{
    public partial class Sales : Form
    {
        ...
        private void guna2Button9_Click(object sender, EventArgs e)
        {
            try
            {
                if (guna2Button3.Checked && guna2DataGridView1.RowCount != 0)
                {
                    string deletionId, query = " ";
                    deletionId = guna2DataGridView1.CurrentRow.Cells[0].Value.ToString();
                    query = "delete from sale where sale_id=" + Int32.Parse(deletionId);
                    QuerySender(query);
        ...
        private void guna2Button10_Click(object sender, EventArgs e)
        {
            if (guna2Button3.Checked && guna2DataGridView1.RowCount != 0)
            {
                ...
                sale_id.Text = guna2DataGridView1.CurrentRow.Cells[0].Value.ToString();
                ...
                string dd = guna2DataGridView1.CurrentRow.Cells[3].Value.ToString();//reading date from grid view and converting it into string
                guna2DateTimePicker1.Value = DateTime.Parse(dd);//converting date string into date datatype and toring it into calender
                Description.Text = guna2DataGridView1.CurrentRow.Cells[4].Value.ToString();
            }
        ...
        {
            if (sale_id.Text.Length != 0)
            {
                try
                {
                    string query = "update sale set amount=" + Int32.Parse(sale_amount.Text) + ",cashier_name='" + cashier_name.Text + "',Sale_Date=TO_DATE('" + guna2DateTimePicker1.Text + "' ,'mm/dd/yyyy'),description='"+Description.Text + "'  where sale_id=" + Int32.Parse(sale_id.Text);
                    QuerySender(query);
                    guna2Button8_Click(sender, e);
                    sale_id.ResetText();
        ...
        private void guna2Button13_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                query = "select * from sale where sale_id=" + sale_idp2.Text;
                if (sale_idp2.Text.Length != 0 && sale_amountp2.Text.Length != 0 && cashier.Text.Length != 0)
                {
                    if (CheckAlreadyProductId(query))
                    {
                        guna2HtmlLabel6.Text = "*This Sale Id already Exits. Please try different Sale Id.";
                        guna2HtmlLabel6.ForeColor = Color.White;
                        sale_idp2.ResetText();
                    }
                    else
                    {
                        query = "insert into sale values(" + sale_idp2.Text + "," + sale_amountp2.Text + ",'" + cashier.Text +"',TO_DATE('" + guna2DateTimePicker2.Text + "' ,'mm/dd/yyyy'),'"  + sales_desc.Text +"')";
                        QuerySender(query);
                        guna2Button8_Click(sender, e);
        ...
        private void sale_amountp2_TextChanged(object sender, EventArgs e)
        {
